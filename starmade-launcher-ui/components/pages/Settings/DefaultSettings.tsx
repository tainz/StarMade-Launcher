...
const SettingRow: React.FC<{
  ...
  <div className="flex justify-between items-center bg-black/20 p-4 rounded-lg border border-white/10">
    ...
    <div className="min-w-[300px] flex justify-end">{children}</div>
  </div>
);

const resolutions = ['1280x720', '1920x1080', '2560x1440', '3840x2160'];

// Special value for "Custom Path..." in Java dropdown
...
const DefaultSettingsForm: React.FC<{ isServer: boolean }> = ({ isServer }) => {
  const [gameDir, setGameDir] = useState(
    isServer
      ? 'C:\\Games\\StarMade\\Servers\\default'
      : 'C:\\Games\\StarMade\\Instances\\default',
  );
  ...
  const { javaVersions } = useData();

  const resolutionOptions = useMemo(
    () => resolutions.map((res) => ({ value: res, label: res })),
    [],
  );

  // Java dropdown options: Auto, discovered Javas, Custom Path...
  ...
  // Selection model for Java dropdown + custom path
  const [javaSelection, setJavaSelection] = useState<string>(() => {
    if (!javaPath) return '';
    const known = javaVersions.find((j) => j.path === javaPath);
    return known ? known.path : CUSTOM_JAVA_VALUE;
  });
  const [customJavaPath, setCustomJavaPath] = useState(
    javaSelection === CUSTOM_JAVA_VALUE ? javaPath : '',
  );
  ...
  return (
    <div className="space-y-4">
      <SettingRow
        ...
        description="The default folder where new installations will be created."
      >
      ...
      {!isServer && (
        <SettingRow
          title="Resolution"
          description="The default screen resolution for new installations."
        >
          <CustomDropdown
            className="w-full"
      ...
      {isServer && (
        <SettingRow
          title="Port"
          description="The default network port for new servers."
        >
          <input
            type="text"
            value={port}
      ...
      <SettingRow
        ...
        description="Set the default RAM allocated to new instances."
      >
      ...
      <SettingRow
        title="JVM Arguments"
        description="Java arguments for advanced users. Memory is managed above."
      >
        <textarea
          value={jvmArgs}
      ...
      <SettingRow
        ...
        description="Choose the default Java for new instances, or specify a custom path."
      >
        <div className="flex flex-col gap-2 w-full">
          <CustomDropdown
            ...
            value={javaSelection}
            onChange={(val) => {
              setJavaSelection(val);
              if (val === '') {
                // Auto: clear javaPath; let launch logic auto-select
                setJavaPath('');
              } else if (val !== CUSTOM_JAVA_VALUE) {
                // Discovered Java: set javaPath to selected path
                setJavaPath(val);
                setCustomJavaPath('');
              }
              // CUSTOM_JAVA_VALUE case: input below will manage javaPath
            }}
            dropUp
          />
...
const TabButton: React.FC<{
  ...
  >
    {children}
    {isActive && (
      <div className="absolute bottom-0 left-0 w-full h-1 bg-starmade-accent rounded-full shadow-[0_0_8px_0px_#227b86]" />
    )}
  </button>
);
...
const DefaultSettings: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'installations' | 'servers'>(
    'installations',
  );
    ...
    <div className="h-full flex flex-col">
      <div className="flex justify-between items-center mb-6 pb-2 border-b-2 border-white/10 flex-shrink-0">
        ...
        <p className="text-sm text-gray-400">
          Configure default values for new installations and servers.
        </p>
      </div>
      ...
      <div className="flex-grow overflow-y-auto pr-4 -mr-4">
        {activeTab === 'installations' ? (
          <DefaultSettingsForm isServer={false} />
        ) : (
          <DefaultSettingsForm isServer={true} />
        )}
      </div>
    </div>
  );
